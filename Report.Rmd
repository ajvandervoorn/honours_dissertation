...
-should I generate this report in R 4.1.x to do linear gradient stuff more easily?

-Maybe don't use LateX Template, but use bookdown instead, (read book on it). Can copy across ideas from LaTeX template though 

-Do I need TikZ package for \TikZ (formatted) macro to work?
...
###
### ADDITINAL NOTES TO SORT:
###

Talk about specials in linear gradient section
    Similar to section explaining steps of dvir, explain steps of tikz, and then in my work explain exactly what steps it relates to (say my stuff helped with step 2,and step 3 and 4 come later
    Get to "why" earlier in report. Before detailed explanation of dvir, state issues with it (slow, functionality limited like anything in development)
    
   Remember gotta finish dvir driver output

   Need to do a discussion and next steps section for honours report

    Paul's meeting notes - start reading, exploring, and experimenting
    Send Paul my report again once I have examples and shit added
    For honours talk - plan questions I could be asked, like "why not windows?"

    Read last meeting notes
    Reread all the tech notes, grid graphics info etc.
    Update my repo on Github?
      
    Writeup
      Keep updated on Github
      Start with editing my honours notes with what feedback he gave on my last version
      Get examples, start collecting and organising screenshots and things better (like profiling)
    ...
    Google UoA Honours projects, I think Brendon might have put some info on his personal website?
    
    Things to check for last parses through:
    -tense of writing (past tense for exisiting), be consistent with what work was done
    ...
    -suppress any warnings in report output (console is fine), but leave to end in case there are any issues
    
    Questions for Paul:
    -How do you "fail" the report?
...
Need to edit/tidy code like what's use in Thomas Yee's example (tidy, remove comments, check indentation is good, check there's nothing redundant occuring?)

    TikZ
      Tidy up driver file to get "clean" gradient output of colours and stops
      Maybe see if I can parse it to R (with extra package edits) to get it to work, even if it doesn't do the transformations that are required?
...
  Research TeX bounding boxes and baselines myself (how close can we get to getting the real info?)
Change honours writing to have substance!   Make a macro for 'dvir' so can change format of it later?   



FOR NEXT WEEK MEETING:
-CONFIRM ANYTHING ABOUT MY UNDERSTANDING THAT'S UNSURE
-READ MORE LIKE TILL TANTAU'S PGF DOC - ABOUT DVI SPECIALS AND SHIT
Remember I still don't know a lot of how dvir works. Like all the font shit? Read documents Paul sent for the algorithms
    
Need to use Honour report template and get that going!
In background of the project, and describing the package as it was, do I use past tense?


Do I use \TeX and \LaTeX all the time? or just type the letters  normally?
Run report from VM shell (so can actually use dvir package in report)
Does my explanation of `dvir` need to identify changes for \TikZ?
Make sure I push report to my GitHub! Do it regularly!
How to find out all the conventions for things like referencing? Can I reference same report multiple times? surely!
Need to look through all meeting notes (both from me and Paul) to ensure everything is noted down that we tried. Remember some of those things might make good discussion points as to effort that was put in!
Add examples of the code I changed (like in font sweeps - do a before and after)
Find out how font def number works in DVI, how does it stay the same across multiple DVI files?
Go through all meeting notes (both Paul's and mine) and add to write up/actions as required

I need to spend some time revising the basic info of `dvir` like the overall how it works. Spend time walking through function, expecially the font sweeps and kpsewhich stuff and all that. And just how it finds fonts and all that
...
Notes for Paul:
  -Trying to find how can put $x - \mu$ in there
  -Still some examples missing
  -Take old report, ask what his scribbles mean?
  -Generate report! Send off by 8 or 9 or so
  -Read Paul's meeting notes from last time
  -Refer to my meeting progress notes I sent him to flesh out  
  -What would I need to change in terms of narration/writing if these bits were the final report? Like is it actually close, or nah?

Formatting:
  CHange image of mu squared over two to actuall rendering that in PDFLatex
  Indent coding in a bit more
  Start using thesis template
  STart proper bibliography
  Find way of having figure numbers under out put from code chunks (and number them all automatically so can reference them easily)
  Add references - both in their spot in the text and at the end
...
This example uses nine calls to `grid.latex()` and was created by a lecturer using the `dvir` package to help write an assignment.
...
To tackle this, and indeed find out whether this even was the case, the first task was to profile the existing code to let us see where in the package time was being spent.

A note on the profiling: the exact results obtained are specific to the computing environment used. Specific details are provided at the end of this section. The sampling nature of profiling (intermittent recording of the call stack) will give different results every time it is done.
...
We can see the function call stack in figure \@ref(fig:profilingSimpleProfvis_0.2-1). At the bottom is the call to `grid.latex()`, which immediately calls `grid.draw()` which in turn calls `latexGrob()`. This calls `readDVI()` for about the first 20ms, then `dviGrob()` for the remaining time to complete the original `grid.latex()` function call, and so on up the function call stack.
...
The `profvis::profvis()` output for our more complicated example, in figure \@ref(fig:profilingYeeProfvis_0.2-1_highlight) reveals most of the time to create the figure is in `grid.latex()`. Note that the code to draw the arrows and the "(a)" in this example is so quick it occupies the very skinny call stack on the very left of the graph. `grid.latex()` and the functions it calls, on the other hand, take up most of the time required to create the example.

In figure \@ref(fig:profilingYeeProfvis_0.2-1_highlight) some blocks in the call stack have been highlighted - these are related to the `engine$fontDef` operation occuring. This is a part of the "font sweep", as was described in the introduction to `dvir` to run through the DVI file, identify all the fonts used, and find and record the font file locations on the system.

In the top left corner of figure \@ref(fig:profilingYeeProfvis_0.2-1_highlight) we are told the aggregate time spent with `engine$fontDef` is 1890ms. Comparing to the total time of this run of about 2700ms, `dvir` is spending a *lot* of time dealing with these fonts.

What was interesting though was that after the font sweep found all the fonts it needed, the following sweeps for the metric and grid information *also* called `engine$fontDef`. The point of the font sweep is that it finds all the fonts to be used later on. The following metric and grid sweeps should not need to "re-sweep" for the fonts.
...
The effect of this is most obvious in figure \@ref(fig:profilingSimpleProfvis_0.2-1_highlight) which is the same as figure \@ref(fig:profilingSimpleProfvis_0.2-1) but highlighting the time spent in `engine$fontDef`. The wrappers for the font, metric and grid sweeps are `dviFonts()`, `dviMetric()` and `dvigrid()` respectively (sixth call from the bottom). Here we can see nearly all of the time spent in the metric and grid sweeps are actually redoing the font sweep!
...
The font sweep looks in the DVI file for op codes 243 to 246. These are the font definitions and define the name of a font and give it an identifier for referencing in the DVI file when it wants to use that font to display a character. What was happening in `dvir` was that every time the metric or grid sweep came across one of these font definitions, it *also* searched for and recorded the font information.

<!---
Need figure captions here on these code examples

I think in code I changed, it only refered to op code 243. Check if the other ones were also affected (244 to 246). Does font sweep do all four op codes? May need to update this bit depending on outcome
--->
...
Figures \@ref{fig:metricFont_0.2-2} and \@ref{fig:gridFont_0.2-2} show the what the code was changed to in `dvir` version 0.2-2. `op_ignore` is an empty function, so when the metric or grid sweeps comes across that op code, they do nothing.

Unfortunately these changes by themself caused an error when running the functions in the package. This is because one task undertaken before the font sweep is to reset or overwrite the global fonts list (which the font sweep then writes to). The metric and grid sweeps were also doing this even though it was only intended for it to be done by the font sweep. This meant after the font sweep found all the font information it was overwritten by the metric and grid sweeps and so when `dvir` tried to draw the characters there was no font information to refer to.

The resetting of the global fonts list was initiated when they passes op code 247 in the DVI file, which is the preamble at the start of every DVI file.
...
Setting the metric and grid sweeps to do nothing, by way of `op_ignore` when they pass the preamble of the DVI file solved this problem as the global fonts list created by the font sweep is not overwritten.
