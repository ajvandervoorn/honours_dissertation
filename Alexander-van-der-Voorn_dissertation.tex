...
(Knuth 1986), but it is limited in its fonts. Compare the y-axis label
...
limitations, as motivation for the \texttt{dvir} package. The approaches
are replicated here.

(Chang 2014) (Chang, Kryukov, and Murrell 2014)
...
\texttt{grid.latex()}. The function \texttt{lattice::xyplot()} (Sarkar
2008) is used as it is a readily available high-level plotting function
using \texttt{grid} graphics.
...
\texttt{matplotlib.texmanager} (Hunter 2007). When this algorithm is
selected, the following code is added to the header of the DVI file:
