...
streamlines the workflow of making R graphics that include some \TeX{}
output. Rather than making some graphics in R and exporting to \LaTeX{}
itself or an image editor like Photoshop to add \TeX{} or \TeX{}-like
output, this can all be done in R through a simple interface from the
\texttt{dvir} package. Three improvements to the \texttt{dvir} package
are investigated and discussed in this report.

Running functions from the \texttt{dvir} package took a long time. The
speed of the package was increased by making two changes - removing
redundant code that meant the \texttt{dvir} package was searching for
fonts three times instead of once, and allowing font information to be
cached so that once font information was found the \texttt{dvir} package
did not need to search again for that same information.
...
be fed into R. This was not fully implemented as the complexity of the
gradient definition from \Tikz{} resulted in ``warped'' gradient fills
which are not possible in R. This work could continue once support for
these ``warped'' gradients are added to R.
...
One of these algorithms performs particularly well for all tested
examples, including multiline text.
...
another without requiring ``touching up'' outside of R.
...
\KeywordTok{mtext}\NormalTok{(expr, }\DataTypeTok{side =} \DecValTok{2}\NormalTok{, }\DataTypeTok{line =} \DecValTok{3}\NormalTok{, }\DataTypeTok{las =} \DecValTok{1}\NormalTok{)}
\end{Highlighting}
\end{Shaded}

\begin{figure}

{\centering \includegraphics[width=0.5\linewidth]{Alexander-van-der-Voorn_dissertation_files/figure-latex/expressionPlot-1} 
...
and their limitations, in R which can get us closer to the \LaTeX{}
...
{\centering \includegraphics[width=0.4\linewidth]{../Figures/extrafont-embed} 
...
{\centering \includegraphics[width=0.4\linewidth]{../Figures/extrafont-2-embed} 
...
\LaTeX{}, as in figure \ref{bracExample}.
...
graphics, by way of the \texttt{grid.latex()} function. Figure
\ref{fig:plotGridLatex} recreates figure \ref{fig:expressionPlot}, but
...
{\centering \includegraphics[width=0.5\linewidth]{Alexander-van-der-Voorn_dissertation_files/figure-latex/plotGridLatex-1} 
...
on top of \TeX{}, essentially consisting of a lot more macros, creating
a much more user-friendly interface to \TeX{}. For example, \LaTeX{}
allows the creation of a document with numbered sections, title pages
and bibliographies without having to explicitly include complicated
\TeX{} macros.

\subsection{DVI (DeVice Independent)}\label{dvi-device-independent}
...
document on a screen or page. A \TeX{} ``engine'' like \texttt{pdflatex}
normally processes \TeX{} to PDF but it can also produce a DVI file. A
DVI file is a binary file describing the layout of the document. For
example, the height of the page, what characters to display and where,
and the fonts to be used. DVI provides a format that can be read into R
so that we can draw the \TeX{} layout in R. This is what the
\texttt{dvir} package does.
...
The \texttt{dvir} package can be used on a larger scale too.
...
We can see the function call stack in figure \ref{fig:prof1}. On the
fifth line from the bottom is the call to \texttt{grid.latex()}, which
immediately calls \texttt{grid.draw()} which in turn calls
\texttt{latexGrob()}. This calls \texttt{readDVI()} for about the first
30ms, then \texttt{dviGrob()} for the remaining time to the end of the
original \texttt{grid.latex()} function call, and so on up the function
call stack.
...
in figure \ref{fig:prof2}, reveals most of the time spent creating the
figure is in \texttt{grid.latex()}. Note that the code to draw the
arrows and the ``(a)'' in this example is so quick it occupies the very
skinny call stack on the far left of the graph. On the other hand,
\texttt{grid.latex()} and its subsequent function calls take up most of
the time required to produce the example.

In figure \ref{fig:prof2} some blocks in the call stack have been
highlighted light grey. These are related to the
\texttt{engine\$fontDef} operation occurring. This is a part of the
``font sweep'', as was described in section \ref{dvirDesc}.

In the top left corner of figure \ref{fig:prof2} we are told the
aggregate time spent with \texttt{engine\$fontDef} is 3350ms. Compared
to the total time of this run (a total of about 4750ms), \texttt{dvir}
...
\texttt{engine\$fontDef}. As the intention of the font sweep is that it
...
\texttt{engine\$fontDef} in light grey. The wrappers for the font,
metric and grid sweeps are \texttt{dviFonts()}, \texttt{dviMetric()} and
\texttt{dvigrid()} respectively (tenth call from the bottom of the
stack). Here nearly all of the time spent in the metric and grid sweeps
are actually redoing the font sweep.
...
where the metric and grid sweeps also redid the font sweep. The function
\texttt{op\_font\_def} takes the font definition in the DVI file related
to that instance of the op code and searches for and records the font
information.
...
version 0.2-2. The function \texttt{op\_ignore} is an empty function, so
when the metric or grid sweeps comes across that op code, they now do
nothing.
...
Unfortunately these changes alone caused an error when running
...
\caption{The average time spent in the \texttt{grid.latex()} function, metric sweep and grid sweep before and after these changes, over 20 runs of the simple example.}\label{fig:speedUp1Simple}
\end{figure}
...
\caption{The average time spent in the \texttt{grid.latex()} function, metric sweep and grid sweep before and after these changes, over 20 runs of the complicated example.}\label{fig:speedUp1Yee}
\end{figure}
...
In fact, that example's 11 \texttt{grid.latex()} calls define a font 29
...
\begin{Shaded}
\begin{Highlighting}[]
\ControlFlowTok{if}\NormalTok{ (dvir}\OperatorTok{::}\KeywordTok{get}\NormalTok{(}\StringTok{"initFonts"}\NormalTok{) }\OperatorTok{||}\StringTok{ }\KeywordTok{is.null}\NormalTok{(dvir}\OperatorTok{::}\KeywordTok{get}\NormalTok{(}\StringTok{"fonts"}\NormalTok{))) }
\NormalTok{  dvir}\OperatorTok{::}\KeywordTok{set}\NormalTok{(}\StringTok{"fonts"}\NormalTok{, }\KeywordTok{vector}\NormalTok{(}\StringTok{"list"}\NormalTok{, }\DecValTok{255}\NormalTok{))}
\end{Highlighting}
\end{Shaded}
...
Otherwise, the existing font cache remains. An argument,
\texttt{initFonts}, was added to \texttt{grid.latex()} with its default
value being equal to the global option \texttt{dvir.initFonts}.
...
profiling as was detailed in the previous section. For the simple
example the time for the font sweep has reduced by 71\%, with
\texttt{grid.latex()} overall taking 45\% less time. Similar results can
be seen in for the example in figure \ref{fig:yeeExample} - a 76\% and
46\% decrease in the font sweep and \texttt{grid.latex()} respectively.

\begin{figure}

{\centering \includegraphics[width=0.6\linewidth]{Alexander-van-der-Voorn_dissertation_files/figure-latex/speedUp2Simple-1} 

}

\caption{The average time spent in the \texttt{grid.latex()} function and font sweep, over 20 runs of the simple example.}\label{fig:speedUp2Simple}
\end{figure}

\begin{figure}

{\centering \includegraphics[width=0.6\linewidth]{Alexander-van-der-Voorn_dissertation_files/figure-latex/speedUp2Yee-1} 

}

\caption{The average time spent in the \texttt{grid.latex()} function and font sweep, over 20 runs of the complicated example.}\label{fig:speedUp2Yee}
\end{figure}
...
The sampling nature of profiling (intermittent recording of the function
call stack) will give different results every time it is done.

The profiling results are also dependent on the computing environment
with which the \texttt{dvir} package is used.
...
\begin{Shaded}
\begin{Highlighting}[]
\NormalTok{tikzCode <-}\StringTok{ }
\StringTok{  }\KeywordTok{paste}\NormalTok{(}\StringTok{"}\CharTok{\textbackslash{}\textbackslash{}}\StringTok{path (0, 1) node[circle,fill=red!40,draw,thick] (x) \{$}\CharTok{\textbackslash{}\textbackslash{}}\StringTok{mu$\}"}\NormalTok{,}
...
\caption{Using \texttt{grid.tikzpicture()} to make the earlier diagram.}\label{fig:gridTikz}
\end{figure}
...
The alteration to the preamble is once again only necessary due to the
use of \texttt{\textbackslash{}dfrac} from the \texttt{amsmath} package.
...
linear gradient fill, like that in figure \ref{tikzLinGrad}.

The following is an extract of the DVI file when the rectangle above is
generated using the SVG DVI driver included with common \TeX{}
distributions, \texttt{pgfsys-dvisvgm.def}. It has been edited slightly
for readability by adding line breaks and removing the line break
character string \texttt{\{?nl\}}.
...
each other. Some letters, like a lower case y or j, have a
``descender''. A descender is a part of a character that sits below the
baseline.

\begin{figure}

{\centering \includegraphics[width=0.7\linewidth]{Alexander-van-der-Voorn_dissertation_files/figure-latex/unnamed-chunk-15-1} 

}

\caption{The baseline and descender of the letter y.}\label{fig:unnamed-chunk-15}
\end{figure}

The \texttt{grid.text()} function accounts for this baseline so when you
bottom-align text at a certain y-value, the descenders will actually
fall below the y-value we defined, despite the bottom justification.
Unfortunately, \texttt{dvir} does not account for text baselines and
will do any alignment in relation to bounding box of the text, which, in
the case of figure \ref{fig:baselineCombine}, is at the bottom of the
descenders.

\begin{Shaded}
\begin{Highlighting}[]
\NormalTok{texText <-}\StringTok{ "\{}\CharTok{\textbackslash{}\textbackslash{}}\StringTok{LARGE changing to }\CharTok{\textbackslash{}\textbackslash{}}\StringTok{LaTeX\{\}\}"}
\KeywordTok{grid.text}\NormalTok{(}\StringTok{"An example of "}\NormalTok{, }
          \DataTypeTok{y =} \FloatTok{0.1}\NormalTok{, }
          \DataTypeTok{just =} \KeywordTok{c}\NormalTok{(}\StringTok{"right"}\NormalTok{, }\StringTok{"bottom"}\NormalTok{), }
          \DataTypeTok{gp =} \KeywordTok{gpar}\NormalTok{(}\DataTypeTok{fontsize =} \DecValTok{18}\NormalTok{))}
\KeywordTok{grid.latex}\NormalTok{(texText, }\DataTypeTok{y =} \FloatTok{0.1}\NormalTok{, }\DataTypeTok{just =} \KeywordTok{c}\NormalTok{(}\StringTok{"left"}\NormalTok{, }\StringTok{"bottom"}\NormalTok{))}
\KeywordTok{grid.lines}\NormalTok{(}\DataTypeTok{x =} \KeywordTok{c}\NormalTok{(}\FloatTok{0.1}\NormalTok{, }\DecValTok{1}\NormalTok{), }\DataTypeTok{y =} \FloatTok{0.1}\NormalTok{, }\DataTypeTok{gp =} \KeywordTok{gpar}\NormalTok{(}\DataTypeTok{lwd =} \FloatTok{0.5}\NormalTok{))}
\end{Highlighting}
\end{Shaded}

\begin{figure}

{\centering \includegraphics[width=0.7\linewidth]{Alexander-van-der-Voorn_dissertation_files/figure-latex/baselineCombine-1} 

}

\caption{Combining \texttt{grid.text()} and \texttt{grid.latex()}.}\label{fig:baselineCombine}
\end{figure}

To fix this, \texttt{dvir} needs to obtain or calculate a value for the
baseline for any given piece of text to use as an offset for the
bounding box alignment when it is rendering the text. From this point
on, this offset will be referred to as a baseline value. All of
\texttt{dvir}'s information comes from the DVI file so it either needs
to extract a baseline value from the DVI file itself, or calculate it
\emph{from} information in the DVI file. Unfortunately plain DVI files
do not state a baseline value, rather they only detail specific
placement of each character. As such, some possible methods to obtain a
baseline value were explored. These methods are referred to as
algorithms from here on due to their heuristic nature.

\subsection{Implementation}\label{implementation}

To explore the algorithms detailed below and evaluate their usefulness
an R function, \texttt{baselines()}, has been created. This function can
be found in the appendix in the file \texttt{algorithms.R}. This
function takes several arguments, including the baseline selection
algorithm as detailed below, any additional information needed for that
particular algorithm, and the \TeX{} code as you would use with
\texttt{grid.latex()}. The output of this function is the baseline value
or values. These values are returned as \texttt{grid} units as they are
distances from the bottom of the bounding box of the text to the text's
baseline.

Once the baseline values have been calculated the bounding box of the
text can be bottom-aligned with the y-value specified but then moved
down by the baseline value. This means the baseline of the text, should
the algorithms work correctly, will be aligned at the specified y-value.
...
there will be no upward move before the first character and the baseline
value returned is 0.
...
\vspace{0.3cm}
\settodepth{\dvidepth} {\begin{minipage}{1in}Multiline, with some line wrapping\end{minipage}}%
\settowidth{\dviwidth} {\begin{minipage}{1in}Multiline, with some line wrapping\end{minipage}}%
...
\vspace{0.3cm}
\end{center}
\caption{The baselines as calculated by the \texttt{alex} algorithm.}\label{alexAlgo}
\end{figure}

For plain text this algorithm works quite well. Unfortunately for many
equations, particularly ones with superscripts or subscripts, the first
upward move is \emph{not} to where the baseline is. Note in figure
\ref{alexAlgo} the \texttt{alex} algorithm has calculated the baseline
of the fifth example to be the bottom of the \(\mu\). This demonstrates
that this algorithm is only practical when the first character's
baseline is the one to be aligned, and in the case of the second
example, does not serve much purpose at all.
...
taking the location after the second vertical cursor move, this
algorithm keeps track of \emph{all} the up and down moves of the cursor.
The motivation behind this is that the upward and downward moves in the
DVI file should reflect the cursor moving to the baseline of the next
character to be typeset. Once again we assume that the first downward
move after the ``HiResBoundingBox'' statement is from the top left to
the bottom left of the bounding box so this algorithm only returns the
upward and downward moves from there. As DVI files have the ability to
save the current cursor location, move around a bit, then reset back to
the saved location, all up and down moves are tracked from the start of
the DVI file.
...
\item
  ...
  baseline value being returned means a method for choosing which
  baseline value is required.
\item
  There are often several up and downward moves in the DVI file between
  typeset characters so in between the ``useful'' baseline values there
  can be some that are duplicates or are not very useful.
\end{itemize}

To account for these considerations, along with the \texttt{dviMoves}
algorithm, the \texttt{baselines()} function also allows a choice of
method to select a \emph{single} baseline out of the many returned by
the algorithm.
...
by the algorithm. This is useful for drawing the text with multiple
lines representing all the baseline values calculated from this method.
...
\item
  ...
  after the first instance of that character
\item
  If multiple cursor moves are made before a character is drawn, the
  earlier moves are still recorded but with an empty character label
\item
...
\begin{figure}
\begin{center}
\settodepth{\dvidepth} {{\LARGE{testing}}}%
\settowidth{\dviwidth} {{\LARGE{testing}}}%
\raisebox{\dimexpr -\dvidepth+0sp}{\makebox[0in][l]{\rule{\dviwidth}{.1pt}}}%
\raisebox{\dimexpr -\dvidepth+220273sp}{\makebox[0in][l]{\rule{\dviwidth}{.1pt}}}%
\raisebox{\dimexpr -\dvidepth+0sp}{\makebox[0in][l]{\rule{\dviwidth}{.1pt}}}%
\raisebox{\dimexpr -\dvidepth+0sp}{\makebox[0in][l]{\rule{\dviwidth}{.1pt}}}%
\raisebox{\dimexpr -\dvidepth+0sp}{\makebox[0in][l]{\rule{\dviwidth}{.1pt}}}%
\raisebox{\dimexpr -\dvidepth+963145sp}{\makebox[0in][l]{\rule{\dviwidth}{.1pt}}}%
\raisebox{\dimexpr -\dvidepth+963145sp}{\makebox[0in][l]{\rule{\dviwidth}{.1pt}}}%
{\LARGE{testing}}

\vspace{0.3cm}
\settodepth{\dvidepth} {{\LARGE{$\sum_{n=1}^{\infty} 2^{-n} = 1$}}}%
\settowidth{\dviwidth} {{\LARGE{$\sum_{n=1}^{\infty} 2^{-n} = 1$}}}%
\raisebox{\dimexpr -\dvidepth+0sp}{\makebox[0in][l]{\rule{\dviwidth}{.1pt}}}%
\raisebox{\dimexpr -\dvidepth+890763sp}{\makebox[0in][l]{\rule{\dviwidth}{.1pt}}}%
\raisebox{\dimexpr -\dvidepth+0sp}{\makebox[0in][l]{\rule{\dviwidth}{.1pt}}}%
\raisebox{\dimexpr -\dvidepth+690925sp}{\makebox[0in][l]{\rule{\dviwidth}{.1pt}}}%
\raisebox{\dimexpr -\dvidepth+690925sp}{\makebox[0in][l]{\rule{\dviwidth}{.1pt}}}%
\raisebox{\dimexpr -\dvidepth+0sp}{\makebox[0in][l]{\rule{\dviwidth}{.1pt}}}%
\raisebox{\dimexpr -\dvidepth+0sp}{\makebox[0in][l]{\rule{\dviwidth}{.1pt}}}%
\raisebox{\dimexpr -\dvidepth+0sp}{\makebox[0in][l]{\rule{\dviwidth}{.1pt}}}%
\raisebox{\dimexpr -\dvidepth+279964sp}{\makebox[0in][l]{\rule{\dviwidth}{.1pt}}}%
\raisebox{\dimexpr -\dvidepth+690925sp}{\makebox[0in][l]{\rule{\dviwidth}{.1pt}}}%
\raisebox{\dimexpr -\dvidepth+279964sp}{\makebox[0in][l]{\rule{\dviwidth}{.1pt}}}%
\raisebox{\dimexpr -\dvidepth+0sp}{\makebox[0in][l]{\rule{\dviwidth}{.1pt}}}%
\raisebox{\dimexpr -\dvidepth+0sp}{\makebox[0in][l]{\rule{\dviwidth}{.1pt}}}%
\raisebox{\dimexpr -\dvidepth+0sp}{\makebox[0in][l]{\rule{\dviwidth}{.1pt}}}%
\raisebox{\dimexpr -\dvidepth+1149677sp}{\makebox[0in][l]{\rule{\dviwidth}{.1pt}}}%
\raisebox{\dimexpr -\dvidepth+1149677sp}{\makebox[0in][l]{\rule{\dviwidth}{.1pt}}}%
{\LARGE{$\sum_{n=1}^{\infty} 2^{-n} = 1$}}

\vspace{0.3cm}
\settodepth{\dvidepth} {{\LARGE{$\sum\limits_{n=1}^{\infty} 2^{-n} = 1$}}}%
\settowidth{\dviwidth} {{\LARGE{$\sum\limits_{n=1}^{\infty} 2^{-n} = 1$}}}%
\raisebox{\dimexpr -\dvidepth+0sp}{\makebox[0in][l]{\rule{\dviwidth}{.1pt}}}%
\raisebox{\dimexpr -\dvidepth+1468012sp}{\makebox[0in][l]{\rule{\dviwidth}{.1pt}}}%
\raisebox{\dimexpr -\dvidepth+1468012sp}{\makebox[0in][l]{\rule{\dviwidth}{.1pt}}}%
\raisebox{\dimexpr -\dvidepth+726142sp}{\makebox[0in][l]{\rule{\dviwidth}{.1pt}}}%
\raisebox{\dimexpr -\dvidepth+1336941sp}{\makebox[0in][l]{\rule{\dviwidth}{.1pt}}}%
\raisebox{\dimexpr -\dvidepth+726142sp}{\makebox[0in][l]{\rule{\dviwidth}{.1pt}}}%
\raisebox{\dimexpr -\dvidepth+726142sp}{\makebox[0in][l]{\rule{\dviwidth}{.1pt}}}%
...
\raisebox{\dimexpr -\dvidepth+0sp}{\makebox[0in][l]{\rule{\dviwidth}{.1pt}}}%
\raisebox{\dimexpr -\dvidepth+726142sp}{\makebox[0in][l]{\rule{\dviwidth}{.1pt}}}%
\raisebox{\dimexpr -\dvidepth+1137103sp}{\makebox[0in][l]{\rule{\dviwidth}{.1pt}}}%
\raisebox{\dimexpr -\dvidepth+726142sp}{\makebox[0in][l]{\rule{\dviwidth}{.1pt}}}%
\raisebox{\dimexpr -\dvidepth+0sp}{\makebox[0in][l]{\rule{\dviwidth}{.1pt}}}%
\raisebox{\dimexpr -\dvidepth+0sp}{\makebox[0in][l]{\rule{\dviwidth}{.1pt}}}%
\raisebox{\dimexpr -\dvidepth+0sp}{\makebox[0in][l]{\rule{\dviwidth}{.1pt}}}%
\raisebox{\dimexpr -\dvidepth+1872150sp}{\makebox[0in][l]{\rule{\dviwidth}{.1pt}}}%
\raisebox{\dimexpr -\dvidepth+1872150sp}{\makebox[0in][l]{\rule{\dviwidth}{.1pt}}}%
{\LARGE{$\sum\limits_{n=1}^{\infty} 2^{-n} = 1$}}

\vspace{0.3cm}
\settodepth{\dvidepth} {{\LARGE{The equation is $x + \frac{\mu^2}{2}$}}}%
\settowidth{\dviwidth} {{\LARGE{The equation is $x + \frac{\mu^2}{2}$}}}%
\raisebox{\dimexpr -\dvidepth+0sp}{\makebox[0in][l]{\rule{\dviwidth}{.1pt}}}%
\raisebox{\dimexpr -\dvidepth+390519sp}{\makebox[0in][l]{\rule{\dviwidth}{.1pt}}}%
\raisebox{\dimexpr -\dvidepth+865870sp}{\makebox[0in][l]{\rule{\dviwidth}{.1pt}}}%
\raisebox{\dimexpr -\dvidepth+1151260sp}{\makebox[0in][l]{\rule{\dviwidth}{.1pt}}}%
\raisebox{\dimexpr -\dvidepth+865870sp}{\makebox[0in][l]{\rule{\dviwidth}{.1pt}}}%
\raisebox{\dimexpr -\dvidepth+865870sp}{\makebox[0in][l]{\rule{\dviwidth}{.1pt}}}%
\raisebox{\dimexpr -\dvidepth+660528sp}{\makebox[0in][l]{\rule{\dviwidth}{.1pt}}}%
...
\raisebox{\dimexpr -\dvidepth+0sp}{\makebox[0in][l]{\rule{\dviwidth}{.1pt}}}%
\raisebox{\dimexpr -\dvidepth+0sp}{\makebox[0in][l]{\rule{\dviwidth}{.1pt}}}%
\raisebox{\dimexpr -\dvidepth+1573603sp}{\makebox[0in][l]{\rule{\dviwidth}{.1pt}}}%
\raisebox{\dimexpr -\dvidepth+1573603sp}{\makebox[0in][l]{\rule{\dviwidth}{.1pt}}}%
{\LARGE{The equation is $x + \frac{\mu^2}{2}$}}

\vspace{0.3cm}
\settodepth{\dvidepth} {{\LARGE{$\frac{\mu^2}{2} + x$ is the equation}}}%
\settowidth{\dviwidth} {{\LARGE{$\frac{\mu^2}{2} + x$ is the equation}}}%
\raisebox{\dimexpr -\dvidepth+0sp}{\makebox[0in][l]{\rule{\dviwidth}{.1pt}}}%
\raisebox{\dimexpr -\dvidepth+865870sp}{\makebox[0in][l]{\rule{\dviwidth}{.1pt}}}%
\raisebox{\dimexpr -\dvidepth+1151260sp}{\makebox[0in][l]{\rule{\dviwidth}{.1pt}}}%
\raisebox{\dimexpr -\dvidepth+865870sp}{\makebox[0in][l]{\rule{\dviwidth}{.1pt}}}%
...
\raisebox{\dimexpr -\dvidepth+1573603sp}{\makebox[0in][l]{\rule{\dviwidth}{.1pt}}}%
\raisebox{\dimexpr -\dvidepth+1573603sp}{\makebox[0in][l]{\rule{\dviwidth}{.1pt}}}%
{\LARGE{$\frac{\mu^2}{2} + x$ is the equation}}

\vspace{0.3cm}
\settodepth{\dvidepth} {{\LARGE{\begin{minipage}{1.5in}Multiline, with some line wrapping\end{minipage}}}}%
\settowidth{\dviwidth} {{\LARGE{\begin{minipage}{1.5in}Multiline, with some line wrapping\end{minipage}}}}%
\raisebox{\dimexpr -\dvidepth+0sp}{\makebox[0in][l]{\rule{\dviwidth}{.1pt}}}%
\raisebox{\dimexpr -\dvidepth+3103857sp}{\makebox[0in][l]{\rule{\dviwidth}{.1pt}}}%
\raisebox{\dimexpr -\dvidepth+3103857sp}{\makebox[0in][l]{\rule{\dviwidth}{.1pt}}}%
\raisebox{\dimexpr -\dvidepth+1662065sp}{\makebox[0in][l]{\rule{\dviwidth}{.1pt}}}%
\raisebox{\dimexpr -\dvidepth+1662065sp}{\makebox[0in][l]{\rule{\dviwidth}{.1pt}}}%
\raisebox{\dimexpr -\dvidepth+220273sp}{\makebox[0in][l]{\rule{\dviwidth}{.1pt}}}%
\raisebox{\dimexpr -\dvidepth+220273sp}{\makebox[0in][l]{\rule{\dviwidth}{.1pt}}}%
\raisebox{\dimexpr -\dvidepth+0sp}{\makebox[0in][l]{\rule{\dviwidth}{.1pt}}}%
...
\raisebox{\dimexpr -\dvidepth+0sp}{\makebox[0in][l]{\rule{\dviwidth}{.1pt}}}%
\raisebox{\dimexpr -\dvidepth+0sp}{\makebox[0in][l]{\rule{\dviwidth}{.1pt}}}%
\raisebox{\dimexpr -\dvidepth+3890289sp}{\makebox[0in][l]{\rule{\dviwidth}{.1pt}}}%
\raisebox{\dimexpr -\dvidepth+3890289sp}{\makebox[0in][l]{\rule{\dviwidth}{.1pt}}}%
{\LARGE{\begin{minipage}{1.5in}Multiline, with some line wrapping\end{minipage}}}

\vspace{0.3cm}
\end{center}
\caption{The baselines as calculated by the \texttt{dviMoves} algorithm.}\label{dviMovesAlgo}
\end{figure}

This algorithm performs very well. In figure \ref{dviMovesAlgo} nearly
every part of all examples has a baseline value associated with it,
including the superscripts of math equations and every line of the
multiline text. This means alignment would be possible with these parts
of the examples.
...
\vspace{0.3cm}
\end{center}
\caption{The baselines as calculated by the \texttt{preview} algorithm.}\label{previewAlgo}
\end{figure}

This algorithm also performs very well. In all examples in figure
\ref{previewAlgo} the natural baseline of the text has been identified
and so this algorithm would be suitable in most use cases. For the
multiline text, the second line of text has been identified as having
``the'' baseline.
...
\vspace{0.3cm}
\end{center}
\caption{The baselines as calculated by the \texttt{dvipng} algorithm.}\label{dvipngAlgo}
\end{figure}

It may not be obvious in printed form in figure \ref{dvipngAlgo} but the
only example with which the \texttt{dvipng} algorithm has selected a
``good'' baseline value is one of the equations. The other examples have
baseline values which are at least slightly off. This is likely a side
effect of this algorithm returning the baseline value in (whole) pixels
which does not lend itself well to this detailed application.
...
best. While these algorithms (except \texttt{dvipng}) perform well for
some or most of the examples, the \texttt{dviMoves} algorithm performs
well for \emph{all} the examples. Notably, it gives the option to align
the baseline of \emph{any} line of multiline text or with almost any
character in a mathematical equation, whether it be of a different size,
part of a fraction, or a superscript or subscript.

The difficulty with the algorithm is how to choose a \emph{single}
baseline value from all the ones calculated. It is possible to manually
check them all the baseline values to find the right one but an
automated selection method would be desirable. As the \texttt{preview}
algorithm calculated baseline values for all examples except the
multiline text, the algorithms would work well together - an
implementation of this functionality in \texttt{dvir} could calculate
the baseline values with both the \texttt{dviMoves} and \texttt{preview}
algorithms with the default selection being the baseline value from
\texttt{dviMoves} that is closest to the baseline value calculated by
the \texttt{preview} algorithm. This could be overridden to select
another baseline value calculated by \texttt{dviMoves}.
...
this project, many use cases are accommodated.

The speed improvement of the package has made the experience using
\texttt{dvir} better when creating graphics with many
\texttt{grid.latex()} calls and also when \emph{developing} graphics
with \texttt{grid.latex()} which require rerunning code with small
tweaks often to get the perfect graphical result. The progress on
aligning \texttt{grid.latex()} text output with its baseline means it is
possible to professionally display text and equations when they are
integrated with other text or graphical features.

Of course, there are still many things which could improve \texttt{dvir}
in future. Now that the font sweeping by \texttt{dvir} is much more
efficient other parts of the code are becoming the ``slowest''. In
particular the reading and writing to disk, of the DVI file for example,
takes more time now relative to the rest of the code.

Implementation of a text baseline algorithm into \texttt{grid.latex()}
itself to align text would also be a useful step forward. In the
instance of the \texttt{dviMoves} algorithm, care is needed when
choosing a method to determine the baseline value and should allow a
user to manually override the choice as well. Additional testing of the
algorithms could also be done to consider more advanced use cases, for
example multiline text with mathematical equations and altered line
spacing. Work on the \Tikz{} gradient fills could also progress as R
expands its support of the gradient fills.
...
3.1.9a}.
\href{https://ctan.org/pkg/pgf\%0A\%20\%20\%20}{https://ctan.org/pkg/pgf
  }.
