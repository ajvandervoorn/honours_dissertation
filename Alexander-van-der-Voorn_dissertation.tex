...
alignment with the bounding box of the text, which, in the case of
figure \ref{fig:baselineCombine}, is at the bottom of the descenders.
...
\KeywordTok{grid.text}\NormalTok{(}\StringTok{"An example of "}\NormalTok{, }\DataTypeTok{y =} \FloatTok{0.1}\NormalTok{, }\DataTypeTok{just =} \KeywordTok{c}\NormalTok{(}\StringTok{"right"}\NormalTok{, }\StringTok{"bottom"}\NormalTok{), }\DataTypeTok{gp =} \KeywordTok{gpar}\NormalTok{(}\DataTypeTok{fontsize =} \DecValTok{18}\NormalTok{))}
...
\texttt{dvir}'s information comes from the DVI file so we either need to
extract a baseline value from the DVI file itself, or calculate it
...
placement of each character, so some possible methods to obtain a
...
This function can be found in the R script file \texttt{algorithms.R},
in the appendix.
