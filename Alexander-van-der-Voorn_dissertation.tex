...
\begin{enumerate}
  ...
  \item The DVI file is read into R. As DVI files are binary they are not easily readable by humans but the \texttt{dvir} function \texttt{readDVI()} translates the DVI file into readable text. This is an extract of a DVI file after using \texttt{readDVI()}, stating the definition of a font and then \textit{setting}, that is, printing, a character ``2''.
\end{enumerate}
...
each other. Some letters, like a lower case \emph{y} or \emph{j}, have a
\emph{descender}. A descender is a part of a character that sits below
the baseline.
...
\caption{The baseline and descender of the letter \textit{y}}\label{fig:unnamed-chunk-16}
\end{figure}
...
\KeywordTok{grid.lines}\NormalTok{(}\DataTypeTok{y =} \FloatTok{0.1}\NormalTok{, }\DataTypeTok{gp =} \KeywordTok{gpar}\NormalTok{(}\DataTypeTok{lwd =} \FloatTok{0.5}\NormalTok{))}
\end{Highlighting}
\end{Shaded}
...
distances from the bottom of the bounding box of the text to its
baseline.
...
\vspace{0.3cm}
\end{center}
\caption{The baselines as calculated using the \texttt{alex} algorithm}\label{alexAlgo}
\end{figure}
...
\vspace{0.3cm}
\end{center}
\caption{Baselines as calculated using the \texttt{dviMoves} algorithm}\label{dviMovesAlgo}
\end{figure}
...
\vspace{0.3cm}
\end{center}
\caption{Baselines as calculated using the \texttt{preview} algorithm}\label{previewAlgo}
\end{figure}
...
\vspace{0.3cm}
\end{center}
\caption{Baselines as calculated using the \texttt{dvipng} algorithm}\label{dvipngAlgo}
\end{figure}
