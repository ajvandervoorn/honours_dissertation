...
did not need to search again for that font.
...
In fact, that example's 11 \texttt{grid.latex()} calls define fonts 29
...
DVI file should reflect the cursor moving to the baseline value of the
next character to be typeset. Once again we assume that the first
downward move after the ``HiResBoundingBox'' statement is from the top
left to the bottom left of the bounding box so this algorithm only
returns the upward and downward moves from there. As DVI files have the
ability to save the current cursor location, move around a bit, then
reset back to the saved location, all up and down moves are tracked from
the start of the DVI file.
...
\item
  ...
  can be some which are duplicates or are not very useful.
\end{itemize}
...
\tightlist
\item
  talk about how text baselines is almost there\ldots{}.
\item
  for coee speed, now it is ``reading/writing to disk'' thast is slow
\end{itemize}
