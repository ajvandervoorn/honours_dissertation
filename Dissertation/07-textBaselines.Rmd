...
Change colour and opacity of these baselines?
--->
...
baselineFunc <- function(testText, algo, caption) {
  for (i in seq_along(testText)) {
    if (i == 1)
      algorithmBaselines <- baselines(tex = testText[i], 
                                           algorithm = algo)
    else
  ...
  latexCode <- c(latexCode, baselineEnviroFoot,caption)
  latexCode
}

alexLatex <- baselineFunc(testText, "alex", "caption here...")
...
\newcommand{\boxreport}[2]{%
  \sbox{\dvibox}{\parbox[b]{1.5in}{#1}}% Change width to be '1in' if going back to better way of doing it. Better yet, get variable that takes width of text itself (like TeX examples from Paul) 
