...
```{r baselineDemo, fig.cap = "Using the `baselines()` function, returning the baseline of $x - \\mu$ with the `alex` algorithm"}
...
```{r baselineAlex, echo = FALSE, results = "asis"}
alexLatex <- baselineFunc(testText, "alex", "Using the `alex` method to calculate the baseline.")
cat(alexLatex)
```
...
<!---
Somehow get unique values of baselines here?
--->

```{r baselineDviMoves, echo = FALSE, results = "asis"}
dviMovesLatex <- baselineFunc(testText, "dviMoves", "Using the `dviMoves` method to calculate the baseline.")
cat(dviMovesLatex)
...
This algorithm uses the `preview` package in \LaTeX{}. [Reference TuG report here] When this algorithm is selected, the following code is added to the header of the DVI file:
...
```{r baselinePreview, echo = FALSE, results = "asis"}
previewLatex <- baselineFunc(testText, "preview", "Using the `preview` method to calculate the baseline.")
cat(previewLatex)
```
...
The `dvipng` program is used to turn DVI files into PNG images. An option of `dvipng` is `--depth` which returns the baseline value in pixels. [reference dvipng package - maybe where you egt it from linux?]
...
```{r baselineDvipng, echo = FALSE, results = "asis"}
dvipngLatex <- baselineFunc(testText, "dvipng", "Using the `dvipng` method to calculate the baseline.")
cat(dvipngLatex)
```
...
Of the algorithms considered, the `dviMoves` algorithm performed best. While most of these algorithms perform well for some or most of the examples, the `dviMoves` algorithm performs well for *all* the examples, notably for giving the option to align the baseline of *any* line of multi-line text. Utilising the fact it returns all baseline values for all characters, it is possible to align, for example, with any character in a mathematical equation, whether it be of a different size or a superscript or subscript.
