...
This algorithm uses the `preview` package [@man-preview] in \LaTeX{}. @preview-tug describes the use of the algorithm for `dvisvgm`, and it has been implemented here as it has in `matplotlib.texmanager` [@matplotlib]. When this algorithm is selected, the following code is added to the header of the DVI file:
