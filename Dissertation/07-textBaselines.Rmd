...
```{r, echo = FALSE, fig.cap = "The baseline and descender of the letter \\textit{y}", fig.height = 0.8, fig.width = 3}
yAlign <- 0.25
...
This function can be found in the R script file `algorithms.R`, in the appendix. 
...
<!---
Need to plot these things with latex to see if DVI moves is actuall best? And if it works for all situations? I think it is....

Need to remove any reference to dviMoves going to random places - they are all logical, just repetitive
...
<!---
```{r baselineFix, fig.cap = "Combining \\texttt{grid.text()} and \\texttt{grid.latex()}, adjusting for the baseline", fig.height = 0.7, fig.width = 4}
# baselineVal <- baselines(texText,
#                          algorithm = "dviMoves",
#                          dviMovesMethod = "index",
#                          dviMovesSelection = 2)
# yOffset <- unit(0.1, "npc") - baselineVal
# grid.text("An example of ", y = 0.1, just = c("right", "bottom"), gp = gpar(fontsize = 18))
# grid.latex(texText, y = yOffset, just = c("left", "bottom"))
# grid.lines(y = 0.1, gp = gpar(lwd = 0.5))
```

Adjusting the y-value in the call to `grid.latex()` by subtracting the calculated baseline value, figure \@ref(fig:baselineFix) vertically aligns the two parts of the text from \@ref(fig:baselineCombine).
--->