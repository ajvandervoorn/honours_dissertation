...
The difference is stark and @murrell-dvir describes [@murrell-dvir] several approaches in R which can get us closer to the \LaTeX{} result, and their limitations. These examples are replicated here.
...
Figure \@ref(fig:latticeGridLatex) recreates figure \@ref(fig:expressionPlot) with the axis titles made using `grid.latex()`. The function `lattice::xyplot()` is used as it is a readily available high-level plotting function using `grid` graphics.
