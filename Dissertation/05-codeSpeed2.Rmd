...
<!---
For complicated example, maybe work out how many times same font is defined in different grid.latex() calls?

Check if same font is used in SAME grid.latex call, does it need to work harder?
...
fnt_def_1    fontnum=17, checksum=-1209964637, scale=786432, design=786432,
             fontname=cmmi12
```
In fact, the example's 11 `grid.latex()` calls define fonts 29 times in total, for eight unique fonts.
...
For talk - the information we actually store about fonts (the hex code chunk from DVI file)
  + To do this the easiest way was to expand the stored information about fonts in `fonts` to include the hex code chunk (from DVI) of the font definition

  + Then we check if all parts of the new hex code chunk are the same as the existing
  
Also for talk, need to find WHY this storing of hex coce stuff takes ages, or is the other steps where DVIR searches for the fonts on system? Look at "findTexFontFile" function perhaps?
--->
...
<!---
Check width/height of these plots compared to plots in previous section (want bars the same width)
--->

```{r speedUp2Simple, fig.cap = "The average time spent in the \\texttt{grid.latex()} function and font sweep for each of the 20 runs of the simple example.", echo = FALSE, fig.width = 5, fig.height = 3.5}
...
```{r speedUp2Yee, fig.cap = "The average time spent in the \\texttt{grid.latex()} function and font sweep for each of the 20 runs of the complicated example", echo = FALSE, fig.width = 5, fig.height = 3.5}
plotProfiling1("yee", versionLabels2, results = results2, levels = levels2)
```

Similar results can be seen in for the example in figure \@ref(fig:yeeExample) - a 76% and 46% decrease in the font sweep and `grid.latex()` respectively.
