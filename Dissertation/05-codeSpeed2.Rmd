...
For example, creating figure \ref{yeeExample} requires three `grid.latex()` calls for the same symbol $\omega$. Each time, the following font definition appears in the DVI file for `grid.latex()` to search for during its font sweep.
...
<!---
Check width of these plots compared to plots in previous section (want bars the same width)
--->

```{r speedUp2Simple, fig.cap = "The total time spent in the `grid.latex()` function and font sweep", echo = FALSE, fig.width = 5.5, fig.height = 4}
plotProfiling1("simple", versionLabels2, results = results2, levels = levels2)
```

```{r speedUp2Yee, fig.cap = "The total time spent in the `grid.latex()` function and font sweep", echo = FALSE, fig.width = 5.5, fig.height = 4}
